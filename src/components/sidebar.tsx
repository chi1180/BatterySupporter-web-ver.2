...
import {Dashboard, List, Settings, Help, QuestionMark} from "@mui/icons-material";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    if (window.location.href.includes("#")) setSelectedSection(window.location.href.split("#")[1].replaceAll("-", " "));
          ...
          {section_names.map((section_name, index) => (
              <li key={index}
                ...
                >
                  {sectionIcon(index)}
                  <h3 className={"pl-4"}>
                    {section_name[0].toUpperCase() + section_name.slice(1, section_name.length)}
                  </h3>
                </Link>
              </li>
          ))}
